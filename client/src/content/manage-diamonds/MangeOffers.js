...
const ManageOffers = () => {
    ...
    const { diamonds, followers } = formData;
    ...
    useEffect(() => {
        ...
        async function getOffers() {
            const res = await api.get('/store/getOffers')
    ...
    const saveOffers = async () => {
        try {
            ...
            const res = await api.post('/store/offers', formData)
            const x = tableData;
            x.push(res.data)
            setTableData(x)
    ...
    const onEdit = async () => {
        try {
            const res = await api.post('/store/editOffers', { id, diamonds, followers })
    ...
    const deleteOffers = async () => {
        try {
            ...
            const res = await api.delete(`/store/deleteOffers/${id}`)
            setTableData(res.data)
        ...
        <>
                            ...
                            <form onSubmit={e => e.preventDefault()}>
                                <Grid container spacing={5}>
                                    ...
                                    <Grid item xs={12}>
                                        ...
                                        >
                                            {id === '' ? <Button type='submit' variant='contained' size='block' onClick={saveOffers} sx={{ py: 2.5, width: '100%'}}>
                                                Add
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ margin: '10px', padding: '10px' }}>
                    <Button onClick={handleOpen} variant="contained" sx={{ float: 'right' }}>+ Add New Offers</Button>
                </div>
                                    ...
                                    <TableBody>
                                        {tableData.map((item, indx) => (
                                            <TableRow key={indx}>
                                                ...
                                                <TableCell align='center'>{item.diamonds}
                                                &nbsp;&nbsp;
                                                ...
                                                <TableCell align='center'>{""} {item.followers} </TableCell>
                                                <TableCell align='center'>
                                                    <EditIcon style={{ cursor: 'pointer' }} onClick={() => edit(item)} color='info' />
                                                </TableCell>
                                                <TableCell align='center'><DeleteForeverIcon sx={{ color: 'pink' }}
                ...
                >
                    <Box sx={style}>
                        <Card style={{ padding: '10px', margin: '10px', background: 'white', color: 'black' }}>
                            ...
                            <Button variant='contained' sx={{ py: 2.5, background: '#df4759', float: 'right', margin: '10px', width: '50px', height: '10px' }} onClick={deleteOffers}>
                                Delete
                            </Button>
